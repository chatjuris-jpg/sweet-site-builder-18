      ...
      >
        ...
        {imgs.length > 1 && (
          <>
            <button
              type="button"
              aria-label={`Foto anterior de ${produto.title}`}
              onClick={() => go(-1)}
            ...
            <button
              type="button"
              aria-label={`Próxima foto de ${produto.title}`}
              onClick={() => go(1)}
            ...
            <div className="absolute inset-x-0 bottom-4 flex justify-center gap-1.5">
              {imgs.map((src, idx) => (
                <button
                  key={src}
                  type="button"
                  aria-label={`Ver foto ${idx + 1} de ${produto.title}`}
                  onClick={() => setI(idx)}
                  className={`h-1.5 rounded-full transition-all ${
